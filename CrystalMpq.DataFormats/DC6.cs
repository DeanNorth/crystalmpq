...
namespace CrystalMpq.DataFormats
{
    ...
    public class DC6
    {
        ...
        public DC6(Stream stream)
        {
            dc6_file = new byte[stream.Length];
            stream.Read(dc6_file, 0, (int)stream.Length);

            string palette = @"C:\Users\dean\Desktop\ds1\Demo_data\Data\Global\Palette\ACT1\pal.dat";
            act_file = System.IO.File.ReadAllBytes(palette);
        }
        ...
        public DC6(byte[] dc6, byte[] act, byte[] color, int shift)
        {
            dc6_file = dc6;
            act_file = act;
            color_file = color;
